...
from mmd.planners.multi_agent import End2EndPlanner
...
def run_multi_agent_trial(test_config: MultiAgentPlanningSingleTrialConfig):
    ...
    # ============================
    # TODO: this should be a combination of high-low level planner
    diffusion_planner_model_args = {
        ...
        # TODO: mayneed to rewrite this conflict - current is t_range, configuration(center, radius)
    }
    ...
    print(f'whats global model tansforms:{global_model_transforms}')
    ...
    if test_config.single_agent_planner_class == "MPD":
        diffusion_planner_model_args['model_ids'] = reference_agent_model_ids[0]
    ...
    goal_l = [goal_l[i] + global_model_transforms[agent_skeleton_l[i][-1][0]][agent_skeleton_l[i][-1][1]]
              for i in range(num_agents)]
    print('CAUTION. i skip a lot here')
    ...
    for i in range(num_agents):
        ...
        if test_config.single_agent_planner_class == "MPD":
            single_agent_planner_model_args_i["model_ids"] = reference_agent_model_ids[i][0]
        single_agent_planner_l.append(planner_class(**single_agent_planner_l))
...
if __name__ == '__main__':
    ...
    test_config_single_tile.single_agent_planner_class = "MPD" # Or "MPDEnsemble"
